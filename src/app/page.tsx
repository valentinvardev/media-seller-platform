...
export default async function HomePage() {
  ...
  return (
    <div className="min-h-screen bg-white">
      ...
      {/* ════════ HERO — compact ════════ */}
      <section className="relative overflow-hidden speed-lines" style={{ background: "linear-gradient(135deg, #F0F6FF 0%, #ffffff 70%)" }}>
          ...
          <div className="grid grid-cols-1 lg:grid-cols-2 gap-8 items-center">
            ...
            <div>
              <div className="inline-flex items-center gap-2 px-3 py-1 rounded-full text-xs font-semibold mb-4"
                style={{ background: "#E8F3FF", color: "#0057A8", border: "1px solid #BFDBFE" }}>
                <span className="w-1.5 h-1.5 rounded-full animate-pulse-glow" style={{ background: "#0057A8" }} />
                Fotos disponibles ahora
              </div>

              <h1 className="font-display font-800 uppercase leading-none text-gray-900 mb-3"
                style={{ fontSize: "clamp(2.8rem, 7vw, 5rem)", letterSpacing: "-0.01em" }}>
              ...
              <p className="text-gray-500 text-base leading-relaxed mb-6 max-w-md">
                Buscá tu dorsal en el evento, comprá y descargá tus fotos en HD al instante.
              </p>
              ...
              <div className="flex flex-col sm:flex-row gap-3 mb-7">
                {[
                  ...
                  <div key={s.num} className="flex items-center gap-2">
                    <span className="font-display font-800 text-2xl leading-none shrink-0"
                      style={{ color: "#0057A8" }}>{s.num}</span>
                    <span className="text-xs font-semibold text-gray-600 leading-tight">{s.label}</span>
                    {i < 2 && <svg className="w-4 h-4 text-gray-300 shrink-0 hidden sm:block" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2}><path strokeLinecap="round" strokeLinejoin="round" d="M9 5l7 7-7 7"/></svg>}
                  </div>
                ))}
              </div>

              <a href="#eventos"
                className="inline-flex items-center gap-2 px-7 py-3.5 rounded-xl font-display font-700 uppercase tracking-wider text-white text-base transition-all hover:scale-105"
                style={{ background: "linear-gradient(135deg, #0057A8, #003D7A)", boxShadow: "0 6px 20px rgba(0,87,168,0.25)" }}>
                Ver eventos
            ...
            <div className="relative hidden lg:flex items-center justify-center">
              <div className="relative w-full max-w-sm rounded-3xl overflow-hidden shadow-2xl aspect-[4/3]"
                style={{ background: "linear-gradient(135deg, #002D6E 0%, #0057A8 60%, #60A5FA 100%)" }}>
                <div className="absolute inset-0 speed-lines opacity-20" />
                <div className="absolute inset-0 flex flex-col items-center justify-center text-white/70">
                  <svg className="w-20 h-20 mb-3 opacity-50" fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={1}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M3 9a2 2 0 012-2h.93a2 2 0 001.664-.89l.812-1.22A2 2 0 0110.07 4h3.86a2 2 0 011.664.89l.812 1.22A2 2 0 0018.07 7H19a2 2 0 012 2v9a2 2 0 01-2 2H5a2 2 0 01-2-2V9z" />
                    <path strokeLinecap="round" strokeLinejoin="round" d="M15 13a3 3 0 11-6 0 3 3 0 016 0z" />
                  </svg>
                  <p className="text-sm font-medium font-display uppercase tracking-widest opacity-60">Tu foto de carrera</p>
                </div>
              ...
              {/* Floating badge */}
              <div className="absolute -bottom-3 -left-3 bg-white rounded-2xl shadow-xl px-4 py-2.5 flex items-center gap-2 border border-gray-100">
                <div className="w-8 h-8 rounded-full flex items-center justify-center" style={{ background: "#E8F3FF" }}>
                  <svg className="w-4 h-4" style={{ color: "#0057A8" }} fill="none" viewBox="0 0 24 24" stroke="currentColor" strokeWidth={2.5}>
                    <path strokeLinecap="round" strokeLinejoin="round" d="M5 13l4 4L19 7" />
                  </svg>
                </div>
                <div>
                  <p className="text-xs font-bold text-gray-900 leading-tight">Descarga inmediata</p>
                  <p className="text-xs text-gray-400">MercadoPago</p>
                </div>
              </div>
            </div>
          </div>
      ...
      {/* ════════ EVENTS ════════ */}
      <section id="eventos" className="py-12 px-5 bg-gray-50">
      ...
      <section style={{ background: "linear-gradient(135deg, #002D6E 0%, #0057A8 100%)" }} className="py-14 px-5">
        <div className="max-w-4xl mx-auto text-center">
          <p className="font-display font-600 uppercase tracking-widest text-blue-200 text-sm mb-5">Método de pago aceptado</p>
          <div className="flex justify-center mb-6">
            <div className="bg-white rounded-2xl px-8 py-5 inline-flex items-center gap-4 shadow-xl">
...
function EventCard({ col }: { col: Collection }) {
  ...
  return (
    <div className="bg-white rounded-2xl border border-gray-200 overflow-visible hover:shadow-lg hover:border-blue-200 transition-all duration-200 group flex flex-col card-hover">
      ...
      {/* Cover — relative so logo can hang out */}
      <div className="relative h-44 overflow-visible">
        ...
        {/* Circular logo — positioned at bottom of cover, overflowing into content */}
        <div className="absolute -bottom-7 left-1/2 -translate-x-1/2 z-10
          w-14 h-14 rounded-full border-4 border-white shadow-lg overflow-hidden bg-white flex items-center justify-center"
      ...
      {/* Content — top padding accounts for logo overflow */}
      <div className="pt-10 pb-5 px-5 flex flex-col flex-1 text-center">
        ...
        {dateStr && (
          <p className="text-xs font-semibold text-blue-600 mb-1">{dateStr}</p>
        )}
        {col.description && (
