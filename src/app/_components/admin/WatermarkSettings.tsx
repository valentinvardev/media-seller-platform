  ...
  return (
    <div className="flex flex-col gap-4">
      <div
        ...
        style={{ borderColor: status === "error" ? "#ef444450" : "#1e1e35", background: "#07070f", height: "160px" }}
        onClick={() => !busy && inputRef.current?.click()}
      >
        {status === "loading" ? (
          <div className="w-full h-full flex items-center justify-center">
            <div className="w-7 h-7 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: "#f59e0b30", borderTopColor: "#f59e0b" }} />
          </div>
        ) : displayed ? (
          <>
            <div className="absolute inset-0" style={{ backgroundImage: "linear-gradient(45deg, #1e1e35 25%, transparent 25%), linear-gradient(-45deg, #1e1e35 25%, transparent 25%), linear-gradient(45deg, transparent 75%, #1e1e35 75%), linear-gradient(-45deg, transparent 75%, #1e1e35 75%)", backgroundSize: "16px 16px", backgroundPosition: "0 0, 0 8px, 8px -8px, -8px 0px" }} />
        ...
        {busy && (
          <div className="absolute inset-0 flex flex-col items-center justify-center gap-2" style={{ background: "rgba(0,0,0,0.6)" }}>
            <div className="w-7 h-7 rounded-full border-2 border-t-transparent animate-spin" style={{ borderColor: "#f59e0b30", borderTopColor: "#f59e0b" }} />
            <p className="text-xs" style={{ color: "#94a3b8" }}>{status === "uploading" ? "Subiendo..." : "Eliminando..."}</p>
          </div>
        )}
      </div>
      ...
      <div className="flex items-center gap-3">
        ...
        {currentUrl && (
          <button onClick={handleDelete} disabled={busy}
            className="text-xs px-3 py-2 rounded-xl disabled:opacity-50 transition-all"
      ...
      {msg && <p className="text-xs" style={{ color: status === "error" ? "#f87171" : "#34d399" }}>{msg}</p>}
